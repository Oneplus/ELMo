...
class SoftmaxLayer(torch.nn.Module):
  ...
  def __init__(self, output_dim, n_class):
    ...
    self.hidden2tag = torch.nn.Linear(output_dim, n_class)
...
class SampledSoftmaxLayer(torch.nn.Module):
  ...
  def __init__(self, output_dim, n_class, n_samples, use_cuda):
    ...
    self.column_emb = torch.nn.Embedding(n_class, output_dim)
    self.column_emb.weight.data.uniform_(-0.25, 0.25)

    self.column_bias = torch.nn.Embedding(n_class, 1)
  ...
  def forward(self, x, y):
    ...
    tag_scores = (x.matmul(self.current_embed_matrix)).view(y.size(0), -1) + \
  ...
  def update_embedding_matrix(self):
    if self.training:
      ...
      for i, word in enumerate(self.negative_samples):
    ...
    self.current_embed_matrix = self.column_emb.forward(columns).transpose(0, 1)
...
class CNNSoftmaxLayer(torch.nn.Module):
